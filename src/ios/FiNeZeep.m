...
-(void)zip:(CDVInvokedUrlCommand*)command
{
    ...
    @try
    {
        ...
        if([SSZipArchiveZeep createZipFileAtPath:toPath withContentsOfDirectory:fromPath keepParentDirectory:NO withPassword:nil])
        {
...
-(void)unzip:(CDVInvokedUrlCommand*)command
{
    ...
    @try
    {
        ...
        if([SSZipArchiveZeep unzipFileAtPath:fromPath toDestination:toPath overwrite:YES password:nil error:&error delegate:self])
        {
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        }
        else
