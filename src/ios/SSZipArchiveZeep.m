...
@interface SSZipArchiveZeep ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end

@implementation SSZipArchiveZeep
{
    NSString *_path;
    NSString *_filename;
    zipFile _zip;
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(id<SSZipArchiveZeepDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<SSZipArchiveZeepDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path
          toDestination:(NSString *)destination
              overwrite:(BOOL)overwrite
               password:(NSString *)password
                  error:(NSError **)error
               delegate:(id<SSZipArchiveZeepDelegate>)delegate
        progressHandler:(void (^)(NSString *entry, unz_file_info zipInfo, long entryNumber, long total))progressHandler
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
    // Begin opening
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:@"SSZipArchiveZeepErrorDomain" code:-1 userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (unzGoToFirstFile(zip) != UNZ_OK)
    {
        ...
        NSError *err = [NSError errorWithDomain:@"SSZipArchiveZeepErrorDomain" code:-2 userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    do {
        @autoreleasepool {
            ...
            if (nil != err) {
                NSLog(@"[SSZipArchiveZeep] Error: %@", err.localizedDescription);
            }
                ...
                if (fp) {
                    ...
                    if (fileInfo.dosDate != 0) {
                        ...
                        if (attr) {
                            if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
                                ...
                                NSLog(@"[SSZipArchiveZeep] Failed to set attributes - whilst setting modification date");
                            }
                        }
                    }
                    ...
                    if (permissions != 0) {
                        ...
                        if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            NSLog(@"[SSZipArchiveZeep] Failed to set attributes - whilst setting permissions");
                        }
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: d[@"modDate"]} ofItemAtPath:d[@"path"] error:&err]) {
            NSLog(@"[SSZipArchiveZeep] Set attributes failed for directory: %@.", d[@"path"]);
        }
        if (err) {
            NSLog(@"[SSZipArchiveZeep] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
    ...
    if (crc_ret == UNZ_CRCERROR)
    {
        ...
        retErr = [NSError errorWithDomain:@"SSZipArchiveZeepErrorDomain" code:-3 userInfo:userInfo];
    }
    if (error)
    {
        *error = retErr;
...
{
    return [SSZipArchiveZeep createZipFileAtPath:path withFilesAtPaths:paths withPassword:nil];
}
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath{
    return [SSZipArchiveZeep createZipFileAtPath:path withContentsOfDirectory:directoryPath withPassword:nil];
}
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirector{
    return [SSZipArchiveZeep createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirector withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths withPassword:(NSString *)password
{
    BOOL success = NO;
    SSZipArchiveZeep *zipArchive = [[SSZipArchiveZeep alloc] initWithPath:path];
    if ([zipArchive open]) {
...
- (BOOL)close
{
    NSAssert((_zip != NULL), @"[SSZipArchiveZeep] Attempting to close an archive which was never opened");
    zipClose(_zip, NULL);
    return YES;
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime
{
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[SSZipArchiveZeep] MSDOS date masks don't add up");
